...
class Agent():
    ...
    def act(self, state, add_noise=True):
        ...
        with torch.no_grad():
            raw_action = self.actor_local(state).cpu().data.numpy()
            # Rescale action back from tanh range to action range.
            action = np.multiply(raw_action + 1, self.action_range / 2) + self.action_low
